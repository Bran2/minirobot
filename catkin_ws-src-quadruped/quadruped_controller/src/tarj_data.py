...
def keep_gait():
    x_gait = 0.04 * np.ones(40)
    y_gait = np.zeros(40)
    z_gait = -0.2 * np.ones(40)
...
def forward_data():
    ...
    x, y, z = np.zeros(step), np.zeros(step), np.zeros(step)
    theta = np.linspace(0, 2 * math.pi, step)
    for i in range(step):
        x[i] = E_lateral * (theta[i] - math.sin(theta[i]))
        y[i] = 0
        z[i] = h * (1 - math.cos(theta[i])) / 2
    # Correction of cycloid position
    x = x - E_lateral * math.pi + 0.04
    z_error = -0.2
    z = z + z_error

    x_left = np.hstack((x[10: 20], np.linspace(x[19], x[0], 20), x[0: 10]))
    y_left = np.hstack((y[10: 20], np.linspace(y[19], y[0], 20), y[0: 10]))
    z_left = np.hstack((np.linspace(z[9], z[0], 10), z_error * np.ones(20), np.linspace(z[19], z[10], 10)))

    x_right = np.hstack((np.linspace(x[10], x[0], 10), x[0: 20], np.linspace(x[19], x[9], 10)))
    y_right = np.hstack((y[10: 20], np.linspace(y[19], y[0], 20), y[0: 10]))
    z_right = np.hstack((z_error * np.ones(10), z[0: 20], z_error * np.ones(10)))
...
def backward_data():
    ...
    for i in range(step):
        ...
        z[i] = h * (1 - math.cos(theta[i])) / 2
    # Correction of cycloid position
    x = x - E_lateral * math.pi + 0.05
    z_error = -0.2
    z = z + z_error

    x_left = np.hstack((x[10: 20], np.linspace(x[19], x[0], 20), x[0: 10]))
    y_left = np.hstack((y[10: 20], np.linspace(y[19], y[0], 20), y[0: 10]))
    z_left = np.hstack((z_error * np.ones(10), z[0: 20], z_error * np.ones(10)))

    x_right = np.hstack((np.linspace(x[9], x[0], 10), x[0: 20], np.linspace(x[19], x[10], 10)))
    y_right = np.hstack((y[10: 20], np.linspace(y[19], y[0], 20), y[0: 10]))
    z_right = np.hstack((np.linspace(z[9], z[0], 10), z_error * np.ones(20), np.linspace(z[19], z[10], 10)))
    return x_left, y_left, z_left, x_right, y_right, z_right
...
#
#     y1 = np.hstack((np.linspace(0.15, 0.15 + offset_y[0], 3), np.linspace(0.15 + offset_y[0], 0.15 + offset_y[0], 6),
#                     np.linspace(0.15 + offset_y[0], 0.15, 3)))
#     y2 = np.hstack((np.linspace(0.15, 0.15, 2), np.linspace(0.15, 0.15 + offset_y[1], 3),
#                     np.linspace(0.15 + offset_y[1], 0.15 + offset_y[1], 4), np.linspace(0.15 + offset_y[1], 0.15, 3)))
#     y3 = np.hstack((np.linspace(0.15, 0.15, 4), np.linspace(0.15, 0.15 + offset_y[2], 3),
#                     np.linspace(0.15 + offset_y[2], 0.15 + offset_y[2], 2), np.linspace(0.15 + offset_y[2], 0.15, 3)))
#     y4 = np.hstack((np.linspace(0.15, 0.15, 6), np.linspace(0.15, 0.15 + offset_y[3], 3),
#                     np.linspace(0.15 + offset_y[3], 0.15, 3)))
#
#     z1 = np.hstack((np.linspace(0., 0. + offset_z[0], 3), np.linspace(0. + offset_z[0], 0. + offset_z[0], 6),
#                     np.linspace(0. + offset_z[0], 0., 3)))
#     z2 = np.hstack((np.linspace(0., 0., 2), np.linspace(0., 0. + offset_z[1], 3),
#                     np.linspace(0. + offset_z[1], 0. + offset_z[1], 4), np.linspace(0. + offset_z[1], 0., 3)))
#     z3 = np.hstack((np.linspace(0., 0., 4), np.linspace(0., 0. + offset_z[2], 3),
#                     np.linspace(0. + offset_z[2], 0. + offset_z[2], 2), np.linspace(0. + offset_z[2], 0., 3)))
...
#
#     if direction == 1:
#         y1_gait = np.hstack((np.linspace(0.15 + slant_length / 2, 0.15 + slant_length, 10),
#                              np.linspace(0.15 + slant_length, 0.15, 20),
#                              np.linspace(0.15, 0.15 + slant_length / 2, 10)))
#         y2_gait = np.hstack((np.linspace(0.15 - slant_length / 2, 0.15, 10), np.linspace(0.15, 0.15 - slant_length, 20),
#                              np.linspace(0.15 - slant_length, 0.15 - slant_length / 2, 10)))
#     elif direction == -1:
#         y1_gait = np.hstack((np.linspace(0.15 - slant_length / 2, 0.1, 10), np.linspace(0.15 - slant_length, 0.15, 20),
#                              np.linspace(0.15, 0.15 - slant_length / 2, 10)))
#         y2_gait = np.hstack((np.linspace(0.15 + slant_length / 2, 0.15, 10), np.linspace(0.15, 0.15 + slant_length, 20),
#                              np.linspace(0.15 + slant_length, 0.15 + slant_length / 2, 10)))
#
